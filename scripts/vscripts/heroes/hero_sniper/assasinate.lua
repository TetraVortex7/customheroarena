function HeadShot(keys)
	...
	if chance+ability.prng > math.random(100) then
		...
		if assassinate:GetLevel() > 0 and assassinate:IsCooldownReady() then
			local cooldown = assassinate:GetCooldown(-1) * ability:GetSpecialValueFor("assassinate_passive_cooldown") * get_octarine_multiplier(caster) / 100
...
function FireAssassinate(keys)
	...
	if not caster:HasScepter() then
		...
		local enemies = FindUnitsInRadius(caster:GetTeam(), ability:GetCursorPosition(), nil, ability:GetSpecialValueFor("scepter_radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
