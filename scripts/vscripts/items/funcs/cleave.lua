...
function CleaveDD(event) -- Datadriven cleave
	...
	for i,x in pairs(units_in_radius) do
		if x ~= target then
			if x:GetUnitName() == "npc_dota_hero_meepo" then damage = damage / 4 end
			
			if target:IsIllusion() or x:IsIllusion() then 
				print(x, caster, damage*damage_pct/4, damage_type)
				ApplyDamage({ victim = x, attacker = caster, damage = damage*damage_pct/4,	damage_type = DAMAGE_TYPE_PURE }) --deal damage
			else
				ApplyDamage({ victim = x, attacker = caster, damage = damage*damage_pct,	damage_type = DAMAGE_TYPE_PURE }) --deal damage	
			end
		end
	end
end
...
function CleaveLua(event)
	...
	for i,x in pairs(units_in_radius) do
		if x ~= target then
			if target:IsIllusion() or x:IsIllusion() then 
				ApplyDamage({ victim = x, attacker = caster, damage = damage*damage_pct/5,	damage_type = damage_type }) --deal damage
			else
				ApplyDamage({ victim = x, attacker = caster, damage = damage*damage_pct,	damage_type = damage_type }) --deal damage	
			end
		end
	end
end
