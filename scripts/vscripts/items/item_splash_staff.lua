if item_splash_staff == nil then item_splash_staff = class({}) end

LinkLuaModifier("modifier_splash_staff_passive","items/item_splash_staff.lua",LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_splash_staff_splash","items/item_splash_staff.lua",LUA_MODIFIER_MOTION_NONE)


function item_splash_staff:GetIntrinsicModifierName(  )
	return "modifier_splash_staff_passive"
end

if modifier_splash_staff_passive == nil then modifier_splash_staff_passive = class({}) end

function modifier_splash_staff_passive:GetAttributes(  )
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_splash_staff_passive:IsHidden()
	return true
end

function modifier_splash_staff_passive:IsPurgable(  )
	return false
end
...
function modifier_splash_staff_passive:OnCreated(  )
	...
	local modifier = self:GetCaster():AddNewModifier(self:GetCaster(),self:GetCaster(),"modifier_splash_staff_splash",{duration = 10*60^2})
	modifier.ability = ability
end

function modifier_splash_staff_passive:GetModifierBonusStats_Intellect(  )
...
function modifier_splash_staff_passive:GetModifierPreAttack_BonusDamage(  )
	return self:GetAbility():GetSpecialValueFor("damage")
end

function modifier_splash_staff_passive:OnDestroy(  )
	if self:GetCaster():HasModifier("modifier_splash_staff_splash") then
		self:GetCaster():RemoveModifierByName("modifier_splash_staff_splash")
	end
end

if modifier_splash_staff_splash == nil then modifier_splash_staff_splash = class({}) end

function modifier_splash_staff_splash:GetAttributes(  )
	return MODIFIER_ATTRIBUTE_NONE
end

function modifier_splash_staff_splash:DeclareFunctions(  )
	return {MODIFIER_EVENT_ON_ATTACK_LANDED}
end

function modifier_splash_staff_splash:IsHidden()
	return true
end

function modifier_splash_staff_splash:OnAttackLanded( params )
...
function item_splash_staff:OnProjectileHit( target, location )
	local target = target
	local pos = location
	if target then 
